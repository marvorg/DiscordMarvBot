...
function check_prefix(server){
        MongoClient.connect("mongodb://loclahost:27017", function(err, client) {
                if(err) {
                        return console.dir(err);
                }

                var db = client.db('marv')
                var conf = db.collection
...
bot.on('message', async message => {
  ...
  var prefix = check_prefix(server);
  ...
  if(command === 'scp') {
    ...
    if(message.member.hasPermission("MANAGE_GUILD") == true) {
      var t = check_prefix(server);
      if(args[0] == null) {
        message.channel.send('The prefix for this server is' + t);
      }
